...
const Skills: React.FC = () => {
  ...
  return (
    <section
      id="skills"
      className="min-h-screen lg:ml-10 mr-auto w-full sm:w-[90%] bg-[#00050C] text-white px-4 lg:p-8 flex flex-col lg:flex-row relative"
    >
      {/* Description div */}
      <div
        className="flex flex-col relative text-left my-20 w-[95%] sm:w-[80%] lg:w-[60%] xl:w-[40%] max-h-[420px] flex-1 mb-8 md:mb-0 md:mr-8 z-10"
        style={{
      ...
      >
        ...
        <div className={`projects-list max-h-[300px] overflow-y-auto mb-10 lg:mb-4`}>
          <div
            className={`text-left mb-4 w-full lg:w-[80%] transition-opacity duration-500 ${
              textFading ? "fade-out" : "fade-in"
            }`}
          >
            {hoveredText ? (
              <p className="text-left h-[160px] lg:h-auto">{hoveredText}</p>
            ) : (
              <>
                <p className="text-left mb-4">
                  I may be a frontend developer, but the subjects I'm familiar with branch out to
                  many different yet relevant fields. Being a frontend developer requires coding
                  knowledge, of course, but design as well. That's where something like Digital art
                  comes in. The list goes on.
                </p>
                <p className="text-left">
                  Clicking on the button below displays the skills of each field I'm familiar with
                  along with every relevant skill.
                </p>
              </>
            )}
          </div>
      ...
      {/* Skills Div */}
      <div
        className={`flex-1 w-[98%] sm:w-full md:w-[98%] lg:w-[40%] xl:w-[60%] min-h-[260px] md:min-h-[400px] max-h-[420px] my-6 lg:my-20 relative overflow-x-hidden lg:overflow-x-visible`}
      >
        <div
        ...
        <div
          className={`fields-container absolute inset-0 flex flex-col items-center justify-center lg:justify-start transition-all duration-500 ${
            showSkills ? "slide-in" : "slide-out"
          }`}
        >
          <div
            className={`skills-content skill-overflow bg-[#152B4480] w-full overflow-x-auto px-8 py-4 font-semibold lg:rounded-bl-md lg:mr-[3px] mb-12 flex space-x-12 transition-all duration-500 ${
              showSkills ? "fade-in" : "fade-out"
            }`}
          >
            {Object.keys(skillsData).map((category) => (
              <button
                ...
                className={`text-[#75D6FF] transition-colors duration-300 ${
                  selectedCategory === category ? "text-white" : "hover:text-white"
                }`}
              >
                {category}
              </button>
          ...
          {/* Skills Set */}
          <div
            className={`skills-content relative bg-[#152b44c6] lg:max-h-[292px] lg:mr-[3px] p-4 lg:rounded-l-lg flex-1 w-full max-w-4xl overflow-hidden transition-all duration-500`}
          >
            <div className="absolute left-0 top-0 h-full w-1/3 transform -skew-x-12 bg-[#152b4400]"></div>

            {/* Fade effect for skill set */}
            <div
              className={`skill-overflow font-semibold relative z-10 overflow-x-auto flex flex-wrap transition-opacity duration-500 ${
