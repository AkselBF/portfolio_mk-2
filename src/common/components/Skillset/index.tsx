...
interface SkillsGridProps {
  ...
  onHover: (name: string, description: string, color: string, link: string | null) => void;
...
const SkillsSet: React.FC<SkillsGridProps> = ({ skills, onHover, resetTitle }) => {
  ...
  const handleMouseEnter = (name: string, description: string, color: string, link: string | null) => {
    if (screenSize.isLargeScreen) {
      onHover(name, description, color, link); // Hover effect for large screens (above 1280px)
    }
  };

  const handleMouseLeave = () => {
    if (screenSize.isLargeScreen) {
      onHover("All field types", "", "white", null); // Reset hover effect for large screens
    }
  };

  // Handles click for medium and small screens
  const handleClick = (name: string, description: string, color: string, link: string | null) => {
    if (screenSize.isSmallScreen || screenSize.isMediumScreen) {
      if (activeSkill === name) {
        setActiveSkill(null);
        onHover("All field types", "", "white", null);  // Pass null when no skill is active
      } else {
        setActiveSkill(name);
        onHover(name, description, color, link); // Pass the link to the parent
      }
    }
  };
  ...
  const renderHexagons = () => {
    // Check if there are no skills, render 'In Progress'
    if (skills.length === 0) {
      return <div className="in-progress-text">In progress</div>;
    }
            ...
            {group.map((skill, idx) => (
              skill.link ? (
                <a
                  ...
                  onMouseEnter={() => handleMouseEnter(skill.name, skill.description, skill.color, skill.link)} // Trigger hover effect
                  onMouseLeave={handleMouseLeave} // Reset hover effect
                  onClick={() => handleClick(skill.name, skill.description, skill.color, skill.link)} // Handle click on small/medium screens
                >
                ...
                <div
                  key={idx}
                  className={`hexagon-wrapper ${idx === 2 ? 'hexagon-third' : ''}`}
                  onMouseEnter={() => handleMouseEnter(skill.name, skill.description, skill.color, skill.link)} // Hover effect for large screens
                  onMouseLeave={handleMouseLeave} // Reset hover effect for large screens
                  onClick={() => handleClick(skill.name, skill.description, skill.color, skill.link)}// Handle click for small/medium screens
                >
                  <div className={`hexagon ${activeSkill === skill.name && (screenSize.isSmallScreen || screenSize.isMediumScreen) ? 'hexagon-active' : ''}`}>
                    <img 
                      src={activeSkill === skill.name ? skill.iconColor : skill.iconWhite}
