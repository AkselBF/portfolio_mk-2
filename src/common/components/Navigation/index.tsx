      ...
      {isHomePath && (
        <>
          ...
          <div className={`md:hidden fixed right-5 ${scrollPosition > 100 ? 'top-16' : 'top-1/4'} transform -translate-y-1/2 z-50 transition-all duration-300`}>
            ...
            {/* Dropdown menu */}
            {isDropdownOpen && (
              <div className="dropdown-menu transition-all duration-500 ease-in-out absolute right-0 mt-2 w-14 p-1.5 space-y-6 origin-top-right bg-gray-800 bg-opacity-90 rounded-md shadow-lg z-50">
                {sections.map(section => (
                  <button
                    key={section.id}
                    onClick={() => {
